...
\textbf{Value} & \textbf{Mnemonic} & $\popstack$ & $\pushstack$ & \textbf{Description} \vspace{5pt} \\
\linkdest{BLOCKHASH}{}0x40 & $\op{BLOCKHASH}$ & 1 & 1 & Get the hash of the last block. \\
&&&&
...
\midrule
\linkdest{SSTORE}{}0x55 & $\op{SSTORE}$ & 2 & 0 & Save word to storage. \\
&&&& $\st'[I_{\mathrm{a}}]_{\mathbf{s}}[ \mst_{\mathbf{s}}[0] ] \equiv \mst_{\mathbf{s}}[1] $ \\
% &&&&\linkdest{C__SSTORE}{} $C_{\text{\tiny SSTORE}}(\st, \mst) \equiv \begin{cases}
% G_{sset} & \text{if} \quad \mst_{\mathbf{s}}[1] \neq 0 \; \wedge \; \st[I_{\mathrm{a}}]_{\mathbf{s}}[\mst_{\mathbf{s}}[0]] = 0 \\
% G_{sreset} & \text{otherwise}
% \end{cases}$ \\
% &&&&\linkdest{A r}{} $A'_{r} \equiv A_{r} + \begin{cases}
% R_{sclear} & \text{if} \quad \mst_{\mathbf{s}}[1] = 0 \; \wedge \; \st[I_{\mathrm{a}}]_{\mathbf{s}}[\mst_{\mathbf{s}}[0]] \neq 0 \\
% 0 & \text{otherwise}
% \end{cases}$ \\
\midrule
...
&&&& where $x=0$ if the code execution for this operation failed due to an\\
&&&& \hyperlink{Exceptional_Halting_function_Z}{exceptional halting} (or for a \op{REVERT}) $\st' = \varnothing$ or if \\
...
\midrule
\linkdest{SELFDESTRUCT}{}0xff & $\op{SELFDESTRUCT}$ & 1 & 0 & Halt execution and register account for later deletion. \\
...
&&&&\linkdest{C tiny SELFDESTRUCT}{} $C_{\text{\tiny SELFDESTRUCT}}(\st, \mst) \equiv G_{selfdestruct} + \begin{cases}
